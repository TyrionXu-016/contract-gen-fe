    ...
    <div class="flex gap-6 min-h-[calc(100vh-8rem)]">
      ...
      <div class="flex-1">
        ...
        <div v-if="activeMenu === 'home'" class="space-y-8">
          <div
            class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-8 text-white shadow-xl"
          >
          ...
          <div class="flex flex-col lg:flex-row gap-8">
            <div class="lg:w-1/4">
              <div
                class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden sticky top-32"
              >
                <div
                ...
                <div class="space-y-4">
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <span class="font-semibold text-gray-900">{{
                      formatStorage(stats.totalCharacters || 0)
                    }}</span>
                  </div>
                </div>
              </div>
                ...
                <div class="space-y-4">
                  ...
                  >
                    <div class="flex-shrink-0 mt-1">
                      <div
                        :class="`w-8 h-8 rounded-full flex items-center justify-center ${activity.iconBg}`"
                      >
...
const updateStats = () => {
  ...
  stats.value = {
    ...
    todayDocuments: documents.value.filter((doc) => doc.created_at.startsWith(today)).length,
    last7Days: documents.value.filter((doc) => {
      const docDate = new Date(doc.created_at)
      const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
      return docDate > sevenDaysAgo
    }).length,
    totalCharacters: documents.value.reduce(
      (total, doc) => total + (doc.content ? stripHtml(doc.content).length : 0),
      0,
    ),
  }
}
